...
async function toFormattedHistoriq (table) {
  ...
  for (const item of table) {
    ...
    tableau.push({what:await getConducteur(item.who), who:item.what, when:item.when, content:item.content, value:await convertValue(item.value,item.type), type:await convertLabel(item.type), table:item.table});
    }
    }
