...
async function pdfWithQr(id, filePath) {
  ...
  page.drawRectangle({x:(595-390)/2, y:325, width:390, height:390, borderColor:rgb(1,0,0), borderWidth:2}); // Petit cadre
...
page.drawText(numberText, {
  ...
  const xCenterB = (page.getWidth() - Math.min(textWidthB, maxWidthB)) / 2;
...
// Cela présente peu d'interêt d'intégrer le logo d'Euréa à ce document qui n'est de fait pas édité par Euréa
  ...
  page.drawText(headerText,{
    x:xCenter, y:800, size:fontSizeHeader, font, color:rgb(1,1,1)
  });
...
const qrImage = await pdfDoc.embedPng(base64Data);
  const qrDims = qrImage.scale(1);
  page.drawImage(qrImage, {x:70, y : 642, width:qrDims.width, height:qrDims.height});
  ...
  // On a la base; il reste les explications et les identifiants
   ...
   page.drawText(`Version du ${await formatTimestamp(new Date())}`, {x:50, y:35, size:12, font, color:rgb(0,0,0)});
...
//  font,
 // color: rgb(0.7, 0, 0), // rouge foncé
//});
  const explications = `                      Le système de gestion des bennes assistée par informatique a pour but d'automatiser le suivi des bennes, centraliser les données et améliorer la rapidité du traitement des données des bennes. Pour cela, il vous suffit de vous connecter à votre Espace Conducteur à l'aide des identifiants ci-dessus ou en scannant le Qr-Code en haut à gauche. Une fois sur votre interface, vous avez deux options (Enregistrer une benne et Trouver une benne). La première vous permet de signaler la position GPS d'une benne au sytème en inscrivant son n° ou en scannant son Qr-Code (voir le cadre rouge collé sur les bennes). Vous devez scannez ce Qr-Code uniquement à la dépose d'une benne (chez le client ou au dépôt). Merci d'activer votre géolocalisation (GPS) dans les paramètres (et de la désactiver une fois la confirmation affichée). Merci de bien vouloir attendre l'affichage de l'îcône verte pour quitter l'application (celui-ci vous confirme le succès de l'opération). La deuxième option vous permet de trouver une benne à partir de son n°. Par exemple, si vous devez chercher la benne 745, entrez son numéro. Vous verrez ensuite appraître les informations de la benne comme son adresse (il vous suffit de cliquer pour l'afficher dans votre GPS). Merci à vous pour votre coopération.`;
  const fontSizeB = 10;
  const maxWidthB = 500;
...
async function pdfWithQrAll(filePath) {
  ...
  for (const ben of bennes) {
    let id = ben.num;
  const page = pdfDoc.addPage([595,842]); // On crée une nouvelle page
